...
def get_control_disease_otc(N, distances, paths=[BassoonFUS_PLKO, BassoonFUS_318]):
    for dir_idx, direc in enumerate(paths):
        tiff_files = [fname for fname in os.listdir(direc) if fname.endswith('tif')]
        tiff_files = [os.path.join(direc, p) for p in tiff_files]
        all_crops_a, all_crops_b = [], []
        otc_avgs = cp.zeros((2, N))
        otc_stds = cp.zeros((2, N))
        ...
        ot_curve = cp.zeros((len(all_crops_a), N))
        for i, (crop_a, crop_b) in enumerate(zip(all_crops_a, all_crops_b)):
            print(f"Processing image {i + 1} of {len(all_crops_a)}")
            transport_plan, cost_matrix = tplans_pixel_by_pixel([crop_a, crop_b])
            otc_values = []
            for dist in tqdm(distances):
                transported_mass = compute_OTC_v2(transport_plan, cost_matrix, dist)
...
def main():
    ...
    plt.fill_between(common_dist, otcs_avgs[0] - otc_stds[0], otc_avgs[0] + otc_stds[0], facecolor='lightblue', alpha=0.5)
    plt.plot(common_dist, otc_avgs[1], color='lightcoral', label='shFUS-318')
    plt.fill_between(common_dist, otcs_avgs[1] - otc_stds[1], otc_avgs[1] + otc_stds[1], facecolor='lightcoral', alpha=0.5)
